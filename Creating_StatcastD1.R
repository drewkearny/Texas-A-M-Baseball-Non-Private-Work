...
dates <- seq.Date(as.Date("2023-02-17"),as.Date("2023-06-25"),by="day")
...
d1_statcast <- d1_statcast %>%
  rename(pitch_type = details.type.code,
         ...
         pitch_description = details.description,
         ...
         pitch_event = details.event,
         pitch_type_code = details.code,
...
d1_statcast <- d1_statcast %>% dplyr::filter(!pitch_team %in% c("Arizona Diamondbacks", "Boston Red Sox"
                                                                , "Mahoning Valley Scrappers", "West Virginia Black Bears",
                                                                "Frederick Keys", "Trenton Thunder", "Williamsport Crosscutters", "Bristol State Liners" ,
                                                                "Pulaski River Turtles" ,"Bluefield Ridge Runners", "Burlington Sock Puppets","Danville Otterbots",
                                                                "Princeton Whistlepigs","Elizabethton River Riders" ,"Kingsport Axmen" ,"Johnson City Doughboys","Greeneville Flyboys"
                                                                )) %>% dplyr::filter(!batting_team %in% c("Arizona Diamondbacks", "Boston Red Sox"
                                                                                          , "Mahoning Valley Scrappers", "West Virginia Black Bears",
                                                                                          "Frederick Keys", "Trenton Thunder", "Williamsport Crosscutters", "Bristol State Liners" ,
                                                                                          "Pulaski River Turtles" ,"Bluefield Ridge Runners", "Burlington Sock Puppets","Danville Otterbots",
                                                                                          "Princeton Whistlepigs","Elizabethton River Riders" ,"Kingsport Axmen" ,"Johnson City Doughboys","Greeneville Flyboys"))
# TAMU heat map
tam <- d1_statcast %>% dplyr::filter(pitch_team == "Texas A&M Aggies")
...
ggplot(tam,aes(x = platelocside, y = platelocheight)) +
  ...
  ggtitle("Texas A&M Pitch Heatmap", subtitle = "Catcher's Perspective")

# See what pitch types are available
unique(tam$pitch_name)
...
pitch_heatmap_function("Texas A&M",c("Sinker"),"0-0")

# install.packages("openxlsx")
library(openxlsx)

# Save the data frame as an Excel file
write.xlsx(d1_statcast, file = "C:\\Users\\kearn\\R Projects\\Aggie Analytics\\D1_StatcastData.xlsx")